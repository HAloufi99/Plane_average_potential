...
if 0 <= 6 < len(LOCPOT_data):
    line = LOCPOT_data[6].strip()
    
    # Split the line into individual numbers, convert them to integers, and sum them up
    atoms_coord = sum(map(int, line.split()))
    
    # Print the total sum
    print("Total number of atoms", ":", atoms_coord)
...
Nx=int(LOCPOT_data_column[13+atoms_coord*3])
Ny=int(LOCPOT_data_column[14+atoms_coord*3])
Nz=int(LOCPOT_data_column[15+atoms_coord*3])
...
index = 15 + atoms_coord  # Initialize the index to start from LOCPOT_data_column[20]
...
for Z in range(Nz):
    ...
    average_value = plane_sum / (Nx * Ny)
    plane_values.append(average_value)

print("\n Maximum average potential value: ", max(plane_values))
print("\n Minimum average potential value: ", min(plane_values))


#normalize = input("\n Do you want to normalize the potential with the maximum value set to zero? (y/n): ")
#if normalize.lower() == "y":
#    plane_values= plane_values - max(plane_values)
